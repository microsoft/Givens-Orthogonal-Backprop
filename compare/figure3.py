...
def experiment_rotMat(d, bs, G): 
    ...
    torch.cuda.synchronize()
    endFwd.record(); torch.cuda.synchronize()
...
def experiment_linear(d, bs, G):
    ...
    startFwd.record()
    UX = U @ X
    jvp = torch.autograd.backward( UX, G )
    torch.cuda.synchronize()
    endFwd.record()
    torch.cuda.synchronize()
...
def run_linear(d, bs, repeats): 
    times = [] 
    G = torch.zeros((d, bs)).normal_(0, 1) 
     
    for i in range(repeats + 1):
        t = experiment_linear(d, bs, G)
        if i > 0: times.append(t)
...
def main(bs, repeats, exclude_cayley, exclude_fasth, compareLinear):

    print("batch size:", bs, "repeats:", repeats)
    print("| %-10s | %-10s | %-10s | %-10s | %-10s |"%("dimension", "FastH", "Cayley", "RotMat" if not compareLinear else "Torch MM", "RotMat TRR"))
    ...
    for i, d in enumerate(range(64, 64*48+1, 64)): 
        ...
        linear = run_linear(d, bs, repeats)
        
        if not exclude_fasth and not compareLinear: data[i, 0, :] = svd
        if not exclude_cayley and not compareLinear: data[i, 1, :] = cay
        data[i, 2, :] = rotMat if not compareLinear else linear
        data[i, 3, :] = rotMatTeamRR

        print("| %-10i | %-10f | %-10f | %-10f | %-10f |"%(d, data[i, 0, :].mean(), data[i, 1, :].mean(), data[i, 2, :].mean(), data[i, 3, :].mean()))
        np.savez("rotMatComparison_" + str(bs) + "_linear_" + str(compareLinear), data)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exFasth", type=bool, required=False, help="fourth optional argument to exclude Fasth Algorithm")
    parser.add_argument("--compareLinear", type=bool, required=False, help="fifth optional argument to compare rotmatTeamRR against nn.linear")
    
    
    args = parser.parse_args()
    main(args.bs, args.repeats, args.exCayley, args.exFasth, args.compareLinear)
